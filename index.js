...
async function main(){
    ...
    responsedb = await mongo.createConnection("mongodb://localhost:27017/Response");
}
...
app.post("/signin",(req,res)=>{
    ...
    try {
        ...
        connection.query("SELECT * FROM credentials WHERE username = ?",arr[1],(err,result)=>{
            if(err) throw err;
            if(result.length != 0){
...
app.get("/myprofile/:username",async(req,res)=>{
    ...
    async function getattemptedQuizs(){
        let p = new Promise(function (res,req){
            let q = `SELECT * FROM quizes`;connection.query(q,async (err,result)=>{
                if(err) throw err;
                let at = []
...
app.get("/:username/getScore/:quizid",async (req,res)=>{
    ...
    try{
        connection.query(q,async (err,result)=>{
            ...
            const temp = quizdb.model(result[0].username,dbs);
            const qz = await temp.findOne({_id : quizid});
            let score = 0, total = qz.questions.length;
